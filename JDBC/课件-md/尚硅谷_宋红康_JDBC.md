...
- **框架 = 反射+ 注解+设计模式。**学习jdbc是为了更好地理解底层的框架

- 客户端/浏览器端 向服务器端发出请求，服务器端做出反应，对应两种模式B/S：browser server，C/S:Client server

  一些app需要更新才能点击按钮是cs模式开发，需要客户端语言开发，安卓Java，ios swift或者oc，而bs模式，内容更新直接访问url，在服务器上，可以做到适时的更新，不要cs的更新app，语言用到html，可移植性比较好。cs好处在于更好的用户体验

  ![](../../../V79FZVEG3Z]EPJOULY})EU3.png![V79FZVEG3Z]EPJOULY})EU3](../../../V79FZVEG3Z]EPJOULY})EU3.png))

- html、css、Javascript移动端的三板斧，html负责浏览器的结构，页面有便签组成；css使得画面更加炫酷，js页面的行为，（提交表的验证行为），js的库jQuery，除此之外，vue（份额大)，react，ajs主流的前端技术。

- XML，可扩展的标记语言，后端用来写配置文件，应用端可以用在前端， 

- Servlet Java语言，Tomcat的核心组件（另外包括filter（过滤器，过滤信息）、listener（监听器）），当浏览器发生注册或者登录的请求时候，Servlet作用：1、获取用户请求2、处理请求，注册、登录。。3、向浏览器端进行响应

- http协议的理解，面试重点问，404,300响应码

- jsp技术，实现动态的页面展示，根据后台数据动态的显示在浏览器端，实现了servlet接口，本质上也是一种Servlet。El表达式替换jsp表达式，JSTL替换jsp中的脚本片段。

- 会话控制，浏览器端为cookie，服务器端为session，确保前后两次请求为一个浏览器发出，

- Ajax 集成在js，**用来实现异步请求**，注册网站时候，用户名可不可用，百度搜索的提示词，手机地图的左右划走页面显示，

- json 本质上为字符串，比xml更加轻量化，主流替换xml，发送数据跟前端 进行交互，
...
- 共同点都是携带sql语句进入mysql数据库执行sql语句的接口，关系：前者是后者的子接口

- 代码的可读性和可维护性。

- **PreparedStatement 能最大可能提高性能：**
  
  - **DBServer**会对**预编译**语句提供性能优化。因为预编译语句有可能被重复调用，所以<u>语句在被DBServer的编译器编译后的执行代码被**缓存**下来，那么下次调用时只要是**相同的预编译语句就不需要编译**，只要将参数直接传入编译过的语句执行代码中就会得到执行。</u>
  - 在statement语句中,即使是相同操作但因为**数据内容不一样**,所以整个语句本身不能匹配,没有缓存语句的意义.事实是没有数据库会对普通语句编译后的执行代码缓存。这样<u>每执行一次都要对传入的语句编译一次。</u>
  - (语法检查，语义检查，翻译成二进制命令，缓存)
  
- PreparedStatement 可以防止 SQL 注入问题，更安全 

- 除了解决Statement的拼串、sql问题之外，PreparedStatement还有哪些好处呢？

  - 1.PreparedStatement操作Blob的数据，而Statement做不到。

  - 2.PreparedStatement可以实现更高效的批量操作，都是一个格式，校验一次，先缓存起来，后面只需要填充占位符即可，
    *
...
- 集合中的迭代器Iterator方法hasNext()，判断下一个有没有元素；next()，方法表示指针下移，返回下移之后的元素
...
- 对于同时运行的多个事务, 当这些事务访问数据库中相同的数据时, 如果没有采取必要的隔离机制, 就会导致各种并发问题:
  ...
  - **不可重复读**: 对于两个事务T1, T2, T1 读取了一个字段, 然后 T2 **更新**了该字段并提交。之后, T1再次读取同一个字段, 值就不同了。
